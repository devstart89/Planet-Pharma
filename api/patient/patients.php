...
/* Whitelist of sortable columns — MUST match the column order
   ...
   NOTE: index 2 (Facility) is intentionally absent — it's not
   orderable client-side, since it's only shown for centralized
   (doctor/nurse/admin) views and isn't a meaningful sort key. */
$sortable = [
    1 => 'fullname',
    3 => 'p.birthday',
    4 => 'p.birthday',
    5 => $statusExpr,
    6 => 'p.last_prescription_date',
    7 => 'p.last_medical_consult',
];
...
/* ---------- CENTRALIZED ACCESS ----------
   Patient records are centralized across all roles now — health_facility,
   doctor, nurse, and super_admin all see patients from every facility,
   not just their own. No role is scoped by facility_id anymore.
   (Kept as a named flag rather than deleting the branches outright,
   in case a specific role needs to be re-scoped later.) */
$isFacilityScoped = false;
