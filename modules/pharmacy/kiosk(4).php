...
(function () {
    ...
    var printerConfigured = <?= $printer['isConfigured'] ? 'true' : 'false' ?>;
    ...
    function renderTicket(ticket) {
        ...
        async function attemptDirectPrint() {
            if (printerConfig.type === 'wireless') return await trySilentPrint();
            if (printerConfig.type === 'wired') return await tryUsbPrint();
            return { ok: false, reason: 'not_configured' };
        }
        ...
        async function proceedWithPrint() {
            ...
            if (!result.ok) {
                ...
                if (reasonText[result.reason] || (result.reason || '').startsWith('http_')) {
                    diagText = reasonText[result.reason] || ('Printer rejected the job (' + result.reason + ') — using standard print dialog.');
                }
            }
