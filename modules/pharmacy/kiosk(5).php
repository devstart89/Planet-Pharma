...
(function () {
    ...
    function showTicketInButton(btnEl, ticket) {
        ...
        /*
         * Cancel BEFORE printing. This is a LOCAL, UI-only reset: it
         * does not call any backend invalidation endpoint, because
         * kiosk_api.php (as currently integrated) has no such action
         * exposed to the kiosk. The queue row created by walkin_create
         * still exists and will simply go uncalled if never printed —
         * per the brief, we do not invent a cancellation call the
         * existing backend doesn't already support. If server-side
         * invalidation is later added, wire it in right here.
         *
         * Confirmed first: going back without printing means walking
         * away with no physical ticket, so a stray tap here shouldn't
         * silently discard an already-generated queue number.
         */
    ...
    async function printWalkinTicket(ticket, wrapperEl) {
        ...
        try {
            ...
            await attemptDirectPrintOrFallback(ticket);
    ...
    /*
     * Confirmation before cancelling an unprinted ticket. `onConfirm`
     * runs only if the patient taps "Yes, Cancel"; tapping "No, Keep
     * It" (or the backdrop) just closes the dialog and leaves the
     * ticket exactly as it was — no double-binding risk since the
     * Yes/No listeners are removed again as soon as one of them fires.
     */
    var cancelConfirmModal = document.getElementById('cancelConfirmModal');
    // `onConfirmCancel` runs if the patient taps "Yes, Cancel";
    // `onKeepAndPrint` runs if they tap "No, Print It" (or the
    // backdrop) — "keeping" the number means printing it, not just
    // dismissing the dialog and leaving them stuck on the same screen.
    function confirmCancelTicket(onConfirmCancel, onKeepAndPrint) {
        ...
        function onYes() { cleanup(); onConfirmCancel(); }
