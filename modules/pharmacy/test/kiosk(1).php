 ...
 *   - E-Pres Online: patient enters Rx/Transaction No.
...
if ($mode === 'epres' && $_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['rx_no'])) {
    $rxNo = trim($_POST['rx_no']);

    if (!ctype_digit($rxNo)) {
        $error = "Please enter a valid Prescription/Transaction No.";
    } else {
        ...
        $stmt = $conn->prepare("
            SELECT pr.id, pr.status, pr.transmitted_at, pr.medicine_status, hf.pharmacy_id AS routed_pharmacy_id
        ...
        if (!$prescription) {
            ...
            // Already has an active (not-Completed) queue entry today at
            // this branch? Reuse it instead of creating a duplicate.
            $stmt = $conn->prepare("
                SELECT * FROM queues
                WHERE pharmacy_id = ? AND prescription_id = ? AND status != 'Completed' AND DATE(created_at) = CURDATE()
                LIMIT 1
            ");
            ...
            if ($existing) {
                ...
                $category = 'Regular'; // E-Pres Online defaults to Regular
...
<head>
<meta charset="UTF-8">
<title>Pharmacy Kiosk — <?= htmlspecialchars($pharmacy['pharmacy_name']) ?></title>
...
<div class="card kiosk-card p-4 text-center">
    ...
    <?php if ($slip): ?>
        <div id="printTicket">
            ...
            <?php if ($slip['prescription_id']): ?>
                <p class="text-muted mb-0">Rx/Transaction No. <?= htmlspecialchars($slip['prescription_id']) ?></p>
            <?php endif; ?>
    ...
    <?php elseif ($mode === 'epres'): ?>
        ...
        <p class="text-muted">Enter your Prescription/Transaction No. to get your queue number.</p>
        <?php if ($error): ?><div class="alert alert-danger py-2"><?= htmlspecialchars($error) ?></div><?php endif; ?>
        <form method="post" action="<?= htmlspecialchars(withBranch('kiosk.php?mode=epres', $branchSlug)) ?>" class="d-grid gap-2">
            <input type="text" name="rx_no" class="form-control form-control-lg text-center"
                   placeholder="e.g. 1024" autofocus required>
