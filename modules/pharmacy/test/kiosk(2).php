...
if ($mode === 'epres' && $_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['rx_no'])) {
    ...
    if ($rxNo === '') {
        ...
        $stmt = $conn->prepare("
            SELECT pr.id, pr.prescription_number, pr.status, pr.transmitted_at, pr.medicine_status, hf.pharmacy_id AS routed_pharmacy_id
        ...
        if (!$prescription) {
            ...
            // Slip prints the patient-facing prescription_number, not the
            // internal prescriptions.id that queues.prescription_id stores.
